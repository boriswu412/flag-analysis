#!/usr/bin/env python3
"""Run unified proof_protocol (export + inline solve, no type 0/1/2 branching)."""
...
from proof_protocol import proof_protocol_unified


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run unified multi-path protocol verification.",
    )
    ...
    parser.add_argument(
        "--cnf-dir",
        default=None,
        help="CNF export directory (default: cnf_out_unified/<config_stem>/)",
    )
    parser.add_argument(
        "--metrics-dir",
        default=None,
        help="Directory for unified proof metrics TXT (default: next to config file)",
    )
    ...
    try:
        ...
        _, stats = proof_protocol_unified(protocol, start_node, init_state, config, args.t)
