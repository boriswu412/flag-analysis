...
mkdir -p results_txt cnf_out_unified
...
echo "Running $TOTAL job(s) from jobs.txt (unified export + parallel solve)..."
...
export BATCH_JOBS
filter_jobs | parallel -j "$BATCH_JOBS" "${MEMFREE_EXPORT[@]}" --colsep '\t' --line-buffer --joblog run_unified_dimacs.log \
  "$ROOT/scripts/run_one_job_unified_dimacs.sh" {1} {2} {3} {#} "$TOTAL"
...
awk -v total="$TOTAL" '
  ...
  END {
    print ""
    print "  CNF:     cnf_out_unified/"
    print "  Metrics: results_txt/"
    print "  Log:     run_unified_dimacs.log"
  }
' run_unified_dimacs.log
